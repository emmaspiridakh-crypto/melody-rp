...
class Moderation(commands.Cog):
    ...
    async def timeout_cmd(self, ctx: commands.Context, member: discord.Member, duration: str, *, reason: str = None):
        ...
        if delta is None:
            await ctx.send("Λάθος χρόνος βλάκα. Κάνε: 10s / 10m / 1h / 1d")
            return
    ...
    @is_ownership_only()
    async def banrole_cmd(self, ctx: commands.Context, role: discord.Role, *, reason: str = None):
        targets = [m for m in role.members if m.id != ctx.author.id]
        if not targets:
            await ctx.send(f"Κανένα μέλος δεν έχει το role {role.mention}.")
            return

        banned, failed = 0, 0
        status_msg = await ctx.send(f"Γίνεται ban... 0/{len(targets)}")
        for i, member in enumerate(targets, start=1):
            try:
                await member.ban(reason=f"[banrole:{role.name}] {reason or '—'} — από {ctx.author}")
                banned += 1
            except (discord.Forbidden, discord.HTTPException):
                failed += 1
            if i % 5 == 0 or i == len(targets):
                await status_msg.edit(content=f"Γίνεται ban... {i}/{len(targets)}")

        await status_msg.edit(content=f"Ολοκληρώθηκε: {banned} μέλη έγιναν ban, {failed} απέτυχαν (role: {role.mention}).")
